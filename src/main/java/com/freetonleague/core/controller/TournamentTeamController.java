...
public class TournamentTeamController {
    ...
    public ResponseEntity<TournamentTeamProposalBaseDto> getTournamentProposalByTeamId(@RequestParam(value = "tournament_id", required = true) long tournamentId,
                                                                                       @RequestParam(value = "team", required = true) long teamId,
                                                                                       @ApiIgnore @AuthenticationPrincipal User user) {
        return new ResponseEntity<>(restTournamentTeamFacade.getProposalForTournament(tournamentId, teamId, user), HttpStatus.OK);
    }
    ...
    public ResponseEntity<Page<TournamentTeamProposalBaseDto>> getTournamentProposalList(@RequestParam(value = "tournament_id", required = true) long tournamentId,
                                                                                         @ApiIgnore @AuthenticationPrincipal User user) {

        return new ResponseEntity<>(restTournamentTeamFacade.getProposalListForTournament(tournamentId, user), HttpStatus.OK);
    }
}
